...
Baseline repo kanonik semasa selepas sambungan No-Guard Departure ialah **465/465 lulus**. Focused Phase 5 suite `tests/telegram-return-notifications-phase5.test.js` ialah milestone **15/15 lulus**. Temporary installer verification pernah mencapai **17/17 focused** dan **446/446 full**, tetapi test sementara telah dibuang dan angka itu bukan baseline semasa. Focused Phase 4 suite `tests/admin-operational-intelligence-phase4.test.js` kekal milestone **9/9 lulus**. Regression No-Guard dilindungi terutamanya oleh `tests/no-guard-departure-mvp.test.js`, `tests/no-guard-auth-directory-regression.test.js` dan `tests/telegram-return-notifications-phase5.test.js`. Syntax checks:
...
Rollout awal production v2.0.0 menggunakan GAS **Version 24**. Production v2.4.0 semasa ialah GAS **Version 50**, `OUTING_CONFIG_V2_ENABLED=true`, readiness hijau dan source frontend menggunakan cache `2.4.0-r1`. Version 44–46 kekal milestone sejarah sebelum rollout No-Guard Version 47–50. Rollback config-driven segera boleh dibuat dengan menetapkan property kepada `false`; ia mengembalikan laluan legacy tanpa code push atau GAS deployment.
